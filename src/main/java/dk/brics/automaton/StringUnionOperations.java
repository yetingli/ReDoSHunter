...
final public class StringUnionOperations {
	...
	final static class State {
		 ...
		 * <code>null</code>.
		 */
		public State getState(char label) throws InterruptedException {
			if (Thread.currentThread().isInterrupted()) {
				throw new InterruptedException();
			}
		 ...
		 */
		State newState(char label) throws InterruptedException {
			if (Thread.currentThread().isInterrupted()) {
				throw new InterruptedException();
			}
		 ...
		 */
		State lastChild() throws InterruptedException {
			if (Thread.currentThread().isInterrupted()) {
				throw new InterruptedException();
			}
		 ...
		 */
		State lastChild(char label) throws InterruptedException {
			if (Thread.currentThread().isInterrupted()) {
				throw new InterruptedException();
			}
		 ...
		 * state.
		 */
		void replaceLastChild(State state) throws InterruptedException {
			if (Thread.currentThread().isInterrupted()) {
				throw new InterruptedException();
			}
		 ...
		 */
		private static char[] copyOf(char[] original, int newLength) throws InterruptedException {
			if (Thread.currentThread().isInterrupted()) {
				throw new InterruptedException();
			}
		 ...
		 * JDK1.5-replacement of {@link Arrays#copyOf(char[], int)}
		 */
		public static State[] copyOf(State[] original, int newLength) throws InterruptedException {
			if (Thread.currentThread().isInterrupted()) {
				throw new InterruptedException();
			}
		 ...
		 */
		private static boolean referenceEquals(Object[] a1, Object[] a2) throws InterruptedException {
			if (Thread.currentThread().isInterrupted()) {
				throw new InterruptedException();
			}
	 ...
	 */
	public void add(CharSequence current) throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}
	 ...
	 */
	public State complete() throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}
	...
	private static dk.brics.automaton.State convert(State s,
			IdentityHashMap<State, dk.brics.automaton.State> visited) throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}
	 ...
	 */
	public static dk.brics.automaton.State build(CharSequence[] input) throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}
	 ...
	 */
	private boolean setPrevious(CharSequence current) throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}
	 ...
	 */
	private void replaceOrRegister(State state) throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}
	 ...
	 */
	private void addSuffix(State state, CharSequence current, int fromIndex) throws InterruptedException {
		if (Thread.currentThread().isInterrupted()) {
			throw new InterruptedException();
		}
